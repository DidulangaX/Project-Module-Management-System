...
const Progress1 = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (groupId && !incompleteStudents.length) {
      try {
        await axios.put('http://localhost:8000/api/students/progress1', {
          groupId,
          students: updatedStudents.map(({ isValid, ...student }) => ({
            ...student,
            progress1Marks: Number(student.marks), // Convert marks to a number
          })),
        });
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize: '28px', color: '#124E66', marginTop: '20px', fontWeight: 'bold', textTransform: 'uppercase' }}>Enter Progress 1 Marks</h2>
